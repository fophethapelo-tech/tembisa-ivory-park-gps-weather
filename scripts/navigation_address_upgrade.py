...
# Add styling for the extra field on small screens without changing existing layout rules.
# Address formatter: prefer house number + street + suburb + city, with full display_name fallback.
...
# Make Nominatim address searches explicitly address-focused and biased to current area.
...
# Use formatted address in suggestions and selected destination.
...
# Personalize every spoken navigation message without changing warning order.
if 'function navSpeak(' not in s:
    marker="function speak(text,force=false){"
    start=s.find(marker)
    end=s.find('\n', start)
    # Keep existing speak untouched; wrap calls through navSpeak.
...
# Persist navigator name with settings.
